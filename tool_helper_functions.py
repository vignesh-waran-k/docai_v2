...
from typing import DefaultDict, Dict, List, MutableSequence, Sequence, Tuple, Union
    ...
    for pn, page in enumerate(document.pages):
        ...
        for entity in document.entities:
            ...
            for coord in entity.page_anchor.page_refs[
                ...
                ):  # or set(content.lower().split()).intersection(row_keywords):
                    row_coords.append(x)
                    y = round(coord.y * height)
                    ycd = max(ycd, y)
                    y_coord.append(y)
                    # ycd_min_row = min(ycd_min_row, y)
                elif x not in xx:
                    xx.append(x)
        ...
        # store the min row y of table
        # if ycd_min!=math.inf:
        #     max_ycd.append(ycd_min_row)
    ...
    for pgn, page in enumerate(document_fp.pages):
        ...
        for bn, block in enumerate(page.blocks):
            ...
            if not is_table_region(block.layout, ystart, yend, height):
                continue
            activity = re.search(
                r"(^\d.\d+(.|) [a-zA-Z\s]+)|(^Total)|(^Sum[a-zA-Z1-9\.+\s]+)|(^Revenue ([a-z]+))|(^[A-Z]\.[1-9|\s][a-zA-Z0-9\s\.-]+)|(^OPEX ([a-z]+))|(^CAPEX ([a-z]+))|(^Taxonomy ([a-z]+)|^[A-Z]+[a-z]+)",
                block_text,
            )
            if activity:
